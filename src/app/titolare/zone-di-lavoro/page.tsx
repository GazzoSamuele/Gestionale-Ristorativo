...
export default async function ZoneDiLavoroPage() {
  const oggi = new Date();
...
const ticketCucina = await prisma.ordine.count({
...
const numeroInEsaurimento = prodotti.filter((prodotto) => Number(prodotto.quantita) < Number (prodotto.limiteMinimo)).length;
  ...
  return (
    <section className={styles.pagina}>
      <header className={styles.intestazione}>
        <h1 className={styles.titolo}>Zone di lavoro</h1>
      ...
      <section className={styles.cucina}>
        <h2>Cucina</h2>
        <p>{ticketCucina} ticket in coda</p>
        <p>{numeroInEsaurimento} prodotti in esaurimento</p>
      </section>
